      ...
      {showCommentForm && (
        <div className="px-5 pb-5 pt-2 border-t border-surface-200 dark:border-surface-700">
          <form onSubmit={handleSubmitComment} className="flex">
            <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="Add a comment..." className="flex-grow p-2 border border-surface-300 dark:border-surface-600 rounded-l-lg bg-white dark:bg-surface-700 text-surface-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary" />
            <button type="submit" className="px-4 bg-primary hover:bg-primary-dark text-white rounded-r-lg transition-colors">Post</button>
          </form>
        </div>
      )}
    </motion.div>
